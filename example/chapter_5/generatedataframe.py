...
import collections
#data
df = pd.read_csv('./data/ml-100k/data.csv',sep='\t',header=None)
#movie list
df_info = pd.read_csv('./data/ml-100k/item.csv',sep='|',header=None)
movielist = [df_info[1].tolist()[indx]+';'+str(indx+1) for indx in range(len(df_info[1].tolist()))]#Python 3中，range()与xrange()合并为range( )。
